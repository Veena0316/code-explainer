...
function App() {
  ...
  const explainCode = async () => {
    ...
    setResult(null);
    try {
      const prompt = "Analyze this code and return ONLY valid JSON with no extra text or markdown: { \"language\": \"detected language\", \"summary\": \"one line what this code does\", \"explanation\": [{ \"line\": \"code line\", \"desc\": \"what it does\" }], \"concepts\": [\"concept1\"], \"tips\": [\"tip1\"], \"difficulty\": \"Beginner or Intermediate or Advanced\", \"complexity\": { \"time\": \"O(n) etc\", \"space\": \"O(n) etc\", \"score\": 1, \"rating\": \"Simple or Moderate or Complex\" }, \"bugs\": [\"potential bug or none\"] }. Code:\n\n" + code;
  ...
  const copyResult = () => {
    ...
    setTimeout(function() { setCopiedResult(false); }, 2000);
  };
      ...
      <div style={{ background: 'linear-gradient(135deg,#1e1b4b 0%,#020617 100%)', padding: '48px 40px 40px', borderBottom: '1px solid #1e293b', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ display: 'flex', gap: '32px', justifyContent: 'center', flexWrap: 'wrap' }}>
            {[['🌐', 'Multi Language'], ['⚡', 'Instant Results'], ['📊', 'Complexity Score'], ['💡', 'Improvement Tips']].map(function(item, i) {
              return (
        ...
        <div style={{ marginBottom: '24px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
            {examples.map(function(ex, i) {
              return (
                <button key={i} onClick={function() { setCode(ex.code); }}
                  style={{ background: 'rgba(99,102,241,0.1)', color: '#818cf8', border: '1px solid rgba(99,102,241,0.2)', borderRadius: '10px', padding: '8px 16px', fontSize: '13px', cursor: 'pointer', fontWeight: '600' }}>
                  {ex.label}
                </button>
              );
            })}
        ...
        <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: '20px', overflow: 'hidden', marginBottom: '20px', boxShadow: '0 8px 32px rgba(0,0,0,0.4)' }}>
          <div style={{ background: '#020617', padding: '12px 20px', borderBottom: '1px solid #1e293b', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={copyCode}
                style={{ background: copied ? '#10b981' : 'rgba(99,102,241,0.15)', color: copied ? 'white' : '#818cf8', border: '1px solid ' + (copied ? '#10b981' : 'rgba(99,102,241,0.3)'), borderRadius: '8px', padding: '5px 14px', fontSize: '12px', cursor: 'pointer', fontWeight: '600' }}>
                {copied ? '✓ Copied!' : '📋 Copy'}
              </button>
          ...
          <div style={{ display: 'flex' }}>
            <div style={{ background: '#020617', padding: '20px 12px', borderRight: '1px solid #1e293b', minWidth: '40px', textAlign: 'right' }}>
              {(code || '').split('\n').map(function(_, i) {
                return <div key={i} style={{ color: '#334155', fontSize: '13px', lineHeight: '1.6', fontFamily: 'monospace' }}>{i + 1}</div>;
              })}
          ...
          <div style={{ background: '#020617', padding: '8px 20px', borderTop: '1px solid #1e293b', display: 'flex', justifyContent: 'space-between' }}>
            ...
            <span style={{ color: '#334155', fontSize: '12px' }}>Powered by Groq AI</span>
          </div>
        </div>
        ...
        {result && (
          <div>
            ...
            {result.complexity && (
              <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: '20px', padding: '24px', marginBottom: '20px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: '16px' }}>
                  {[['Time', result.complexity.time, '#f59e0b'], ['Space', result.complexity.space, '#a855f7'], ['Score', result.complexity.score + '/10', getScoreColor(result.complexity.score)], ['Rating', result.complexity.rating, getScoreColor(result.complexity.score)]].map(function(item, i) {
                    return (
                      <div key={i} style={{ background: '#020617', borderRadius: '12px', padding: '16px', textAlign: 'center' }}>
                        <div style={{ color: '#64748b', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '8px' }}>{item[0]}</div>
                        <div style={{ color: item[2], fontSize: i < 3 ? '22px' : '16px', fontWeight: '900' }}>{item[1]}</div>
                      </div>
                    );
                  })}
                </div>
            ...
            <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: '20px', padding: '24px', marginBottom: '20px' }}>
              ...
              {result.explanation && result.explanation.map(function(item, i) {
                return (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: '20px' }}>
              <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: '16px', padding: '24px' }}>
                ...
                {result.concepts && result.concepts.map(function(c, i) {
                  return (
              ...
              <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: '16px', padding: '24px' }}>
                ...
                {result.tips && result.tips.map(function(t, i) {
                  return (
